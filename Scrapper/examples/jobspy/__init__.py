...
def scrape_jobs(
    ...
    for site, job_response in site_to_jobs_dict.items():
        for job in job_response.jobs:
            ...
            # Handle Salary (Compensation) more effectively
            compensation_obj = job_data.get("compensation")
            if compensation_obj and isinstance(compensation_obj, dict):
                job_data["salary"] = f"{compensation_obj.get('min_amount', 'N/A')} - {compensation_obj.get('max_amount', 'N/A')} {compensation_obj.get('currency', 'USD')}/{compensation_obj.get('interval').value if compensation_obj.get('interval') else ''}".strip()
            else:
                job_data["salary"] = "Not available"

            # Ensure description exists
            job_data["description"] = job_data.get("description", "No description available")
    ...
    if jobs_dfs:
        ...
        desired_order: list[str] = [
            ...
            "job_type",
            "date_posted",
            "salary",  # Modified to include combined salary field
            "is_remote",
            "num_urgent_words",
            "benefits",
            "emails",
